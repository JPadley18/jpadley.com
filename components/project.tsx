...
export default function Project({ id, thumbnail, title, description, longDescription, gallery, links, icons, modalProps }:
    { id: string, thumbnail: ImageDefinition, title: string, description: string,
        longDescription: string, gallery: ImageDefinition[], links: JSX.Element[], icons: IconDefinition[],
    ...
    return (
        <>
            ...
            <div className="w-3/4 lg:w-1/2 xl:w-1/3 bg-slate-900 border border-gray-200 shadow-lg hover:scale-105 transition-transform shadow-gray-600 mx-auto mt-20 rounded-lg hover:cursor-pointer" onClick={() => modalProps.setOpenModal(id)}>
                <Image src={thumbnail.path} alt={thumbnail.alt} width={thumbnail.width} height={thumbnail.height} className="object-cover w-full h-60 rounded-t-lg" />
                <div className="p-5">
