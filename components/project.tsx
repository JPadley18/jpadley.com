...
export default function Project({ image, title, description, href, icons }: { image: string, title: string, description: string, href: string, icons: IconDefinition[] }) {
    return (
        <div className="w-3/4 sm:w-1/3 bg-slate-900 border border-gray-200 shadow-lg hover:scale-105 transition-transform shadow-gray-600 mx-auto mt-20 rounded-lg">
                <Link href={href}>
                    ...
                    <ul className="pt-5">
                        {icons.map((icon) => (
                            <li key={icon.iconName} className="inline">
                                <FontAwesomeIcon icon={icon} size="3x" className="pr-3" style={{color: "#FFFFFF"}} fixedWidth/>
                            </li>
                        ))}
                    </ul>
                </div>
